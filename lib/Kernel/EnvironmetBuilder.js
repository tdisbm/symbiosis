...
EnvironmentBuilder.prototype =
{
    build : function()
    {
        ...
        this._compiledSet
            ...
            .addItem('nodeBuilder', NodeResolver)
    ...
    _resolveNodes : function()
    {
        ...
        for (var i = 0, n = nodes.length; i < n; i ++) {
            NodeResolver
                .resolve(nodes[i], this._environement.getIo())
                .setIo(this._environement.getIo());
        }

        NodeResolver.linkNodes();
        NodeResolver.getCollection().iterate(function(node) {
            $this._handshakeListener.listen(node);
        });
    },
